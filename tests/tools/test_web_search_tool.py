...
from nomi.agent.tools.web import WebSearchTool
from nomi.config.schema import WebSearchConfig
...
async def test_brave_search(monkeypatch):
    ...
    result = await tool.execute(query="nomi", count=1)
...
async def test_duckduckgo_search(monkeypatch):
    ...
    monkeypatch.setattr("nomi.agent.tools.web.DDGS", MockDDGS, raising=False)
    import nomi.agent.tools.web as web_mod
