...
class TestRestartCommand:
    ...
    async def test_restart_sends_message_and_calls_execv(self):
        from nomi.command.handlers.runtime import cmd_restart
        ...
        with patch.dict(os.environ, {}, clear=False), \
             patch("nomi.command.handlers.runtime.os.execv") as mock_execv:
    ...
    async def test_restart_intercepted_in_run_loop(self):
        ...
        with patch.object(loop, "_dispatch", new_callable=AsyncMock) as mock_dispatch, \
             patch("nomi.command.handlers.runtime.os.execv"):
    ...
    async def test_status_intercepted_in_run_loop(self):
        ...
        with patch.object(loop, "_dispatch", new_callable=AsyncMock) as mock_dispatch:
            ...
            mock_dispatch.assert_not_called()
            out = await asyncio.wait_for(bus.consume_outbound(), timeout=1.0)
            assert "nomi" in out.content.lower() or "Model" in out.content
    ...
    async def test_run_agent_loop_resets_usage_when_provider_omits_it(self):
        ...
        assert loop._state.last_usage["prompt_tokens"] == 9
        assert loop._state.last_usage["completion_tokens"] == 4

        await loop._run_agent_loop([])
        assert loop._state.last_usage["prompt_tokens"] == 0
        assert loop._state.last_usage["completion_tokens"] == 0
