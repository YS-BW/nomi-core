"""Zhipu provider regression tests."""
...
@pytest.mark.asyncio
async def test_zhipu_enables_thinking_for_reasoning_mode() -> None:
...
@pytest.mark.asyncio
async def test_zhipu_disables_thinking_for_minimal_mode() -> None:
    mock_create = AsyncMock(return_value=_fake_chat_response("hello"))
    spec = find_by_name("zhipu")
...
@pytest.mark.asyncio
async def test_zhipu_sets_clear_thinking_false_when_history_has_reasoning_content() -> None:
    mock_create = AsyncMock(return_value=_fake_chat_response("hello"))
    spec = find_by_name("zhipu")
...
@pytest.mark.asyncio
async def test_zhipu_forced_tool_choice_downgrades_to_auto() -> None:
...
@pytest.mark.asyncio
async def test_zhipu_chat_stream_keeps_openai_stream_path() -> None:
    spec = find_by_name("zhipu")
...
def test_zhipu_never_uses_responses_api() -> None:
    spec = find_by_name("zhipu")
    with patch("nomi.providers.backends.openai_compat.AsyncOpenAI"):
        provider = ZhipuProvider(api_key="zhipu-test-key", spec=spec, default_model="glm-4.5")

    assert provider._should_use_responses_api("glm-4.5", "high") is False
