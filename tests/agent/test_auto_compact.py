...
from nomi.command import CommandContext
from nomi.config.schema import AgentDefaults
from nomi.providers.base import LLMResponse


def _make_loop(tmp_path: Path, idle_compact_after_minutes: int = 15) -> AgentLoop:
...
class TestIdleCompactConfig:
    """Test idle compact configuration."""

    def test_default_ttl_is_zero(self):
        """Default TTL should be 0 (disabled)."""
        defaults = AgentDefaults()
        assert defaults.idle_compact_after_minutes == 0

    def test_custom_ttl(self):
        """Custom TTL should be stored correctly."""
        defaults = AgentDefaults(idle_compact_after_minutes=30)
        assert defaults.idle_compact_after_minutes == 30
