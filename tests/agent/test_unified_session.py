...
def _make_loop(tmp_path: Path, unified_session: bool = False) -> AgentLoop:
    ...
    with patch("nomi.agent.loop.SessionManager"), \
         patch("nomi.agent.loop.Dream"):
        loop = AgentLoop(
            bus=bus,
            provider=provider,
            workspace=tmp_path,
...
class TestUnifiedSessionConfig:
    ...
    def test_onboard_generated_config_contains_unified_session(self, tmp_path: Path):
        """save_config() writes 'unifiedSession' into config.json (simulates nomi onboard)."""
...
class TestConsolidationUnaffectedByUnifiedSession:
    ...
    async def test_consolidation_skips_empty_session_for_unified_key(self):
        ...
        from nomi.agent.memory import Consolidator, MemoryStore
...
class TestInterruptWithUnifiedSession:
    ...
    async def test_active_tasks_use_effective_key_in_unified_mode(self, tmp_path: Path):
        ...
        loop._state.active_tasks.setdefault(effective_key, []).append(task)

        # Wait for task to complete
        await task

        # Verify the task is stored under UNIFIED_SESSION_KEY, not the original channel:chat_id
        assert UNIFIED_SESSION_KEY in loop._state.active_tasks
        assert "telegram:123456" not in loop._state.active_tasks
    ...
    async def test_interrupt_session_finds_task_in_unified_mode(self, tmp_path: Path):
        ...
        loop._state.active_tasks[UNIFIED_SESSION_KEY] = [task]
    ...
    async def test_interrupt_session_cross_channel_in_unified_mode(self, tmp_path: Path):
        ...
        loop._state.active_tasks[UNIFIED_SESSION_KEY] = [task1, task2]
