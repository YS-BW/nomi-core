...
echo "nomi line count"
echo "=================="
echo ""

echo "Core runtime"
echo "------------"
core_agent=$(count_top_level_py_lines "nomi/agent")
core_bus=$(count_top_level_py_lines "nomi/bus")
core_config=$(count_top_level_py_lines "nomi/config")
core_cron=$(count_top_level_py_lines "nomi/cron")
core_heartbeat=$(count_top_level_py_lines "nomi/heartbeat")
core_session=$(count_top_level_py_lines "nomi/session")
...
echo "----------------"
extra_tools=$(count_recursive_py_lines "nomi/agent/tools")
extra_skills=$(count_skill_lines "nomi/skills")
extra_api=$(count_recursive_py_lines "nomi/api")
extra_cli=$(count_recursive_py_lines "nomi/cli")
extra_channels=$(count_recursive_py_lines "nomi/channel")
extra_utils=$(count_recursive_py_lines "nomi/utils")
...
echo "Notes"
echo "-----"
echo "  - agent/ only counts top-level Python files under nomi/agent"
echo "  - tools/ is counted separately from nomi/agent/tools"
echo "  - skills/ counts .md, .py, and .sh files"
echo "  - not included here: command/, providers/, security/, templates/, nomi.py, root files"
