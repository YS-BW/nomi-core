...
def test_exec_extract_absolute_paths_captures_home_paths() -> None:
    cmd = "cat ~/.nomi/config.json > ~/out.txt"
    paths = ExecTool._extract_absolute_paths(cmd)
    assert "~/.nomi/config.json" in paths
    assert "~/out.txt" in paths


def test_exec_extract_absolute_paths_captures_quoted_paths() -> None:
    cmd = 'cat "/tmp/data.txt" "~/.nomi/config.json"'
    paths = ExecTool._extract_absolute_paths(cmd)
    assert "/tmp/data.txt" in paths
    assert "~/.nomi/config.json" in paths
...
def test_exec_guard_blocks_home_path_outside_workspace(tmp_path) -> None:
    ...
    error = tool._guard_command("cat ~/.nomi/config.json", str(tmp_path))
...
def test_exec_guard_allows_media_path_outside_workspace(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("nomi.agent.tools.shell.get_media_dir", lambda: media_dir)
...
def test_exec_guard_allows_global_skill_path_outside_workspace(tmp_path) -> None:
    global_skills = tmp_path / ".nomi" / "skills"
...
def test_exec_guard_blocks_windows_drive_root_outside_workspace(monkeypatch) -> None:
    import nomi.agent.tools.shell as shell_mod
...
def test_exec_rejects_sleep_based_scheduling_emulation() -> None:
    ...
    assert error == "Error: delayed shell execution is not allowed; use task_create_after instead"
...
def test_exec_rejects_shell_scheduler_commands() -> None:
    ...
    assert error == "Error: shell scheduling commands are not allowed; use task_create_at/task_create_daily/task_create_every instead"
