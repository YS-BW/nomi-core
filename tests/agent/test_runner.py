...
async def test_runner_calls_hooks_in_order():
    from nomi.agent.hook import AgentHook, AgentHookContext
    from nomi.agent.execution.runner import AgentRunner, AgentRunSpec
...
@pytest.mark.asyncio
async def test_runner_streaming_request_keeps_tool_definitions() -> None:
    ...
    async def chat_stream_with_retry(*, tools=None, **kwargs):
    ...
    tools = MagicMock()
    tools.get_definitions.return_value = [
        {"type": "function", "function": {"name": "task_create_after"}},
    ]
    tools.execute = AsyncMock(return_value="已创建任务")
    ...
    runner = AgentRunner(provider)
    result = await runner.run(AgentRunSpec(
        initial_messages=[{"role": "user", "content": "讲个故事"}],
        tools=tools,
        model="mimo-v2.5",
        max_iterations=1,
        max_tool_result_chars=_MAX_TOOL_RESULT_CHARS,
    ...
    assert result.final_content == "hello"
    assert captured_tools[0] == [{"type": "function", "function": {"name": "task_create_after"}}]
...
async def test_runner_persists_large_tool_results_for_follow_up_calls(tmp_path):
    ...
    assert (tmp_path / ".nomi" / "tool-results" / "test_runner" / "call_big.txt").exists()


def test_persist_tool_result_prunes_old_session_buckets(tmp_path):
    from nomi.agent.execution.tool_results import maybe_persist_tool_result

    root = tmp_path / ".nomi" / "tool-results"
...
def test_persist_tool_result_logs_cleanup_failures(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "nomi.agent.execution.tool_results._cleanup_tool_result_buckets",
        lambda *_args, **_kwargs: (_ for _ in ()).throw(OSError("busy")),
    )
    monkeypatch.setattr(
        "nomi.agent.execution.tool_results.logger.warning",
...
@pytest.mark.asyncio
async def test_runner_no_longer_falls_back_to_task_tool_result_after_empty_finalization() -> None:
    """task 工具执行后如果模型最终答复为空，不再直接回退到工具结果。"""
    from nomi.agent.execution.runner import AgentRunner, AgentRunSpec
    from nomi.utils.runtime import EMPTY_FINAL_RESPONSE_MESSAGE
    ...
    tools = MagicMock()
    tools.get_definitions.return_value = []
    tools.execute = AsyncMock(return_value="已创建自动任务：`task_1`（打开微信）")
...
async def test_runner_executes_tool_calls_returned_by_finalization_retry() -> None:
    ...
    assert result.tools_used == ["task_create_after"]


@pytest.mark.asyncio
async def test_runner_executes_empty_content_tool_calls_returned_by_finalization_retry() -> None:
    """最终补救提示即使返回空正文 + tool_calls，也必须直接执行工具。"""
    from nomi.agent.execution.runner import AgentRunner, AgentRunSpec
...
def test_snip_history_drops_orphaned_tool_results_from_trimmed_slice(monkeypatch):
    ...
    monkeypatch.setattr("nomi.agent.execution.runner.estimate_prompt_tokens_chain", lambda *_args, **_kwargs: (500, None))
    token_sizes = {
        "old user": 120,
        "tool call": 120,
        "tool output": 40,
    ...
    monkeypatch.setattr(
        "nomi.agent.execution.runner.estimate_message_tokens",
...
async def test_runner_keeps_going_when_tool_result_persistence_fails():
    ...
    runner = AgentRunner(provider)
    with patch("nomi.agent.execution.runner.maybe_persist_tool_result", side_effect=RuntimeError("disk full")):
        result = await runner.run(AgentRunSpec(
            initial_messages=[{"role": "user", "content": "do task"}],
            tools=tools,
            model="test-model",
...
async def test_llm_error_not_appended_to_session_messages():
    ...
    from nomi.agent.execution.runner import (
...
async def test_streamed_flag_not_set_on_llm_error(tmp_path):
    ...
    _streamed must NOT be set so channel sender can deliver the error."""
    from nomi.agent.loop import AgentLoop
...
async def test_next_turn_after_llm_error_keeps_turn_boundary(tmp_path):
    from nomi.agent.loop import AgentLoop
    from nomi.agent.execution.runner import _PERSISTED_MODEL_ERROR_PLACEHOLDER
    from nomi.bus.events import InboundMessage
    from nomi.bus.queue import MessageBus
...
async def test_length_recovery_gives_up_after_max_retries():
    ...
    from nomi.agent.execution.runner import _MAX_LENGTH_RECOVERIES, AgentRunner, AgentRunSpec
...
async def test_backfill_missing_tool_results_inserts_error():
    ...
    from nomi.agent.execution.runner import _BACKFILL_CONTENT, AgentRunner
...
async def test_backfill_repairs_model_context_without_shifting_save_turn_boundary(tmp_path):
    ...
    from nomi.agent.loop import AgentLoop
    from nomi.agent.execution.runner import _BACKFILL_CONTENT
    from nomi.bus.events import InboundMessage
    from nomi.bus.queue import MessageBus
...
async def test_runner_backfill_only_mutates_model_context_not_returned_messages():
    ...
    from nomi.agent.execution.runner import _BACKFILL_CONTENT, AgentRunner, AgentRunSpec
...
async def test_microcompact_replaces_old_tool_results():
    ...
    from nomi.agent.execution.runner import _MICROCOMPACT_KEEP_RECENT, AgentRunner
...
async def test_drain_injections_passes_limit_to_callback_when_supported():
    ...
    from nomi.agent.execution.runner import _MAX_INJECTIONS_PER_TURN, AgentRunner, AgentRunSpec
    from nomi.bus.events import InboundMessage
...
async def test_injection_cycles_capped_at_max():
    ...
    from nomi.agent.execution.runner import _MAX_INJECTION_CYCLES, AgentRunner, AgentRunSpec
    from nomi.bus.events import InboundMessage
...
async def test_pending_queue_cleanup_on_dispatch(tmp_path):
    ...
    assert msg.session_key not in loop._state.pending_queues
...
async def test_followup_routed_to_pending_queue(tmp_path):
    ...
    from nomi.agent.loop import UNIFIED_SESSION_KEY
    from nomi.bus.events import InboundMessage
    ...
    loop._state.pending_queues[UNIFIED_SESSION_KEY] = pending
...
async def test_pending_queue_preserves_overflow_for_next_injection_cycle(tmp_path):
    ...
    from nomi.agent.loop import AgentLoop
    from nomi.agent.execution.runner import _MAX_INJECTIONS_PER_TURN
    from nomi.bus.events import InboundMessage
    from nomi.bus.queue import MessageBus
...
async def test_pending_queue_full_falls_back_to_queued_task(tmp_path):
    ...
    loop._state.pending_queues["cli:c"] = pending
...
async def test_dispatch_republishes_leftover_queue_messages(tmp_path):
    ...
    pending = asyncio.Queue(maxsize=20)
    session_key = "cli:c"
    loop._state.pending_queues[session_key] = pending
    ...
    queue = loop._state.pending_queues.pop(session_key, None)
