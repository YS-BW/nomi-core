...
## 0. 当前优先级：Instance 关系系统重构

本阶段目标是在 `nomi-core` 内把 instance 关系从“remote token + pending/relation 混用”重构为“agent card + 一次性邀请码 + 临时申请 + 接受后 relation token”的模型。不引入 `peer` 概念，不修改 `nomi-protocol`，不改 desktop。

当前代码实现状态：代码、文档和局部测试已完成；真实双实例 smoke 尚未执行。
...
- 新增 `InstanceRelationManager`，支持一次性 invite、临时 request、已接受 relation、删除关系和本地权限修改。
