"""Tests for tool hint formatting (nomi.utils.tool_hints)."""
...
class TestToolHintKnownTools:
    ...
    def test_read_file_long_path(self):
        result = _hint([_tc("read_file", {"path": "/home/user/.local/share/uv/tools/nomi/agent/loop.py"})])
    ...
    def test_exec_abbreviates_paths_in_command(self):
        ...
        cmd = "cd D:\\Documents\\GitHub\\nomi\\.worktree\\tomain\\nomi && git diff origin/main...pr-2706 --name-only 2>&1"
        result = _hint([_tc("exec", {"command": cmd})])
    ...
    def test_exec_abbreviates_linux_paths(self):
        ...
        cmd = "cd /home/user/projects/nomi/.worktree/tomain && make build"
        result = _hint([_tc("exec", {"command": cmd})])
        assert "\u2026/" in result
    ...
    def test_exec_abbreviates_home_paths(self):
        ...
        cmd = "cd ~/projects/nomi/workspace && pytest tests/"
        result = _hint([_tc("exec", {"command": cmd})])
        assert result.startswith("$ cd ")
        assert "projects" in result or "\u2026/" in result
    ...
    def test_task_create_after(self):
        result = _hint([_tc("task_create_after", {"instruction": "1 分钟后打开微信"})])
        assert result == 'task_create_after("1 分钟后打开微信")'

    def test_task_delete(self):
        result = _hint([_tc("task_delete", {"task_id": "task_123"})])
        assert result == 'task_delete("task_123")'

    def test_task_update_instruction(self):
        result = _hint([_tc("task_update_instruction", {"task_id": "task_123"})])
        assert result == 'task_update_instruction("task_123")'
